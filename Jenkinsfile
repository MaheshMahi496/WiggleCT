...
node {
   ...
   stage('Check out the code from Git Repository') {// for display purposes
      ...
      url: 'https://github.com/MaheshMahi496/WiggleCT.git'
  ...
  stage('SONAR Analysis') {
   ...
   stage('Function Test Cases execution') {
      // Run the maven build
      if (isUnix()) {
         SoapUIPro(environment: '', pathToProjectFile: 'D:/Wiggle CT/Wiggle-Customer-readyapi-project.xml', pathToTestrunner: 'C:/Program Files/SmartBear/ReadyAPI-2.7.0/bin/testrunner.bat', projectPassword: '', testCase: '', testSuite: '')        
      } else {
         SoapUIPro(environment: '', pathToProjectFile: 'D:/Wiggle CT/Wiggle-Customer-readyapi-project.xml', pathToTestrunner: 'C:/Program Files/SmartBear/ReadyAPI-2.7.0/bin/testrunner.bat', projectPassword: '', testCase: '', testSuite: '')
